              ...
              <form class="space-y-4 md:space-y-6" action="<?= url_to('login') ?>" method="post">
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <?php if (setting('Auth.sessionConfig')['allowRemembering']): ?>
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="remember" name="remember" aria-describedby="remember" type="checkbox" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800" <?php if (old('remember')): ?> checked<?php endif ?>>
                            </div>
                    ...
                    <?php if (setting('Auth.allowMagicLinkLogins')) : ?>
                        <a href="<?= url_to('magic-link') ?>" class="text-sm font-medium text-blue-600 hover:underline dark:text-blue-500"><?= lang('Auth.forgotPassword') ?></a>
                    <?php endif ?>
                ...
                <?php if (setting('Auth.allowRegistration')) : ?>
                    <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                        <?= lang('Auth.needAccount') ?> <a href="<?= url_to('register') ?>" class="font-medium text-blue-600 hover:underline dark:text-blue-500"><?= lang('Auth.register') ?></a>
                    </p>
                <?php endif ?>
              </form>
          </div>
